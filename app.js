...
async function loadFileList() {
  ...
  if (window.NOVEL_DATA && Array.isArray(window.NOVEL_DATA)) {
    console.log("Loading from local NOVEL_DATA");
    state.files = window.NOVEL_DATA.map(item => ({
      path: item.path,
      title: item.title,
      size: item.content.length // Approximation
    })).sort((a, b) => naturalSort(a.path, b.path));

    els.status.textContent = `本地共 ${state.files.length} 章`;
    return;
  }

  // 2. Fallback to Github API for tree
...
async function loadChapter(path) {
  ...
  try {
    let text = "";
    const localData = window.NOVEL_DATA ? window.NOVEL_DATA.find(f => f.path === path) : null;

    if (localData) {
      console.log("Loading chapter from local data:", path);
      text = localData.content;
    } else {
...
async function appendNextChapter() {
  ...
  try {
    ...
    let text = "";
    const localData = window.NOVEL_DATA ? window.NOVEL_DATA.find(f => f.path === file.path) : null;
